...
(async () => {
  ...
  try {
    ...
    const verifyFilled = async (field, expected) => {
       ...
       * Chinese string entirely. This equality then read '' === '' and a field holding いいえ
       * verified as holding はい, and a field holding NOTHING verified as holding either. Same
       * defect as the containment arm in verifyChoiceInContainer, one function apart.
       *
       * When either side normalises away, compare the CLEANED text instead. Raw equality is
       * strictly stricter than normalised equality, and it is only ever reached on the pairs the
       * normalised comparison could not have judged, so no answer that used to verify stops
       * verifying: a non-Latin answer that is genuinely correct still matches itself.
       */
