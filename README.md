...
Supported actions are `click`, `fill`, `fillByLabelText`, `upload`, `waitForSelector`, `press`, `select`, `extract`, `discover`, and `confirmRequired`. Arbitrary caller-supplied JavaScript is rejected.
...
Place `{"type":"confirmRequired","selector":"#application button[type=submit]","maxRetries":1,"contractVersion":1}` immediately before a final submit click with that exact same selector when the portal can paint a value while still showing `This requires an answer`. The selector must resolve to exactly one final submit control. The runner scopes confirmation to that control's closest form, so a newsletter or search form earlier on a multi-form page cannot be inspected in place of the application. It inspects every required control in the application form and retries only answered controls that still expose live invalid state. Text and date inputs receive focus, input, change, and blur; native selects receive the same commit; React Select controls receive an exact control click, Escape, and blur; selected radios use their exact associated label; checked checkboxes receive non-toggling input and change commits; custom selected controls receive one exact click, with their selected semantic state restored and verified if the click is a toggle. Each affected control gets one initial attempt and at most one retry. The result includes `requiredFieldConfirmation` with `version`, `status`, `requiredControls`, `attempts`, `retries`, and `unresolved`. Each attempt carries a durable selector, label, field type, outcome, `attemptCount`, and an optional reason. `attemptCount` is 1 for controls that did not retry and 2 only for an affected control that used the one bounded retry. An empty, selectorless, or unconfirmed required control returns `blocked` and withholds the one final submit click. A final submit without valid version 1 confirmation proof is also withheld.
